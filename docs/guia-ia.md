# Guia d’ús i registre de la IA

Esta és la guia permanent sobre l’ús de la IA. Les consultes reals de cada
alumne es registren exclusivament en [`ai-log.md`](ai-log.md).
...
- agrupa en una sola entrada les preguntes que tinguen un mateix objectiu;
- fes una entrada nova quan canvie l’objectiu, no per cada missatge;
...
- usa resums breus per a l’ajuda rebuda i la resta de camps;
...
Pots transformar allò que realment has comprés en notes reutilitzables dins de
`docs/base-coneixement/`. El registre i la base tenen funcions diferents:

- `ai-log.md` conserva què vas preguntar i quina ajuda vas rebre en cada
  microrepte;
- `base-coneixement/` organitza per temes les explicacions que podràs consultar
  més avant.

Usa un fitxer curt per tema, amb noms descriptius com `sessions.md`,
`validacio.md` o `apis.md`. Escriu amb les teues paraules, inclou exemples
menuts diferents de la solució avaluable i enllaça l’entrada d’`ai-log.md` que
n’és l’origen. No copies converses, respostes completes ni solucions del repte.

La IA pot ajudar-te a agrupar preguntes, detectar temes i ordenar una nota, però
no pot inventar què has comprés ni redactar una reflexió personal que no hages
aportat.
